...
****************************************************/
	$db_server			= 'localhost';				// Database server, usually "localhost", 
 	...
 	$db_name 			= 'rcjonlin_enter';			// Database name, leave empty for 'all databases'
	$db_user 		    = 'rcjonlin_enter';			// Database username
	$db_pass 		    = 'G1,?O0XKm^Gw';			// Database password
...
/***************************************************
	E-mail settings
****************************************************/
 	$website            = 'enter.rcjonline.org.au';		    // Your site's domain (without www. part)
	$send_to 		    = 'evan@baileyfinance.com.au';		        // backup file will be sent to?
	$from 		        = 'no-reply@' . $website;			// some hosting providers won’t let you send backups from invalid e-mail address
...
****************************************************/
    ...
    $full_path      = '/home/rcjonlin/enter.rcjonline.org.au/sys-admin/backup/cron'; 
...
function send_attachment($file, $file_is_db = true) {
	...
	$body .= 'If you are reading this, then your e-mail client probably doesn\'t support MIME.' . "\n";
	$body .= $mailer . "\n";
	$body .= '--' . $boundary . "\n";
